...
def get_cm(x):
    global dist
    dist = round((8700 - sm.get()) *.0343, 1)
    #print(dist,time.ticks_ms())

@rp2.asm_pio(set_init=rp2.PIO.OUT_LOW)
def ping():
    mov(x,osr)       # mov prefilled osr to x
    ...
    label('cm')
    jmp(x_dec,'ck_echo')   # ck for timeout < 300
    jmp('do_delay')        # if so jmp to 80 ms delay
    label('ck_echo')
    jmp(pin,'cm')   	   # while echo high jmp to cm
...
sm = rp2.StateMachine(7, ping, freq=1000000, set_base=trig, jmp_pin=echo, in_base=echo)
sm.irq(get_cm)
sm.put(8700)        # new
sm.exec("pull()")  # lines
sm.active(1)
